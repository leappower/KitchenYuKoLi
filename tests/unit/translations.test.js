/**
 * Unit tests for TranslationManager (src/assets/js/translations.js — IIFE build)
 *
 * translations.js is an IIFE that exposes TranslationManager class and a
 * singleton instance on window. Tests use global.TranslationManager to create
 * fresh instances, and mock global.LANG_REGISTRY so language validation passes.
 */

// Provide a mock LANG_REGISTRY before the IIFE runs so languageNames is non-empty.
beforeAll(() => {
  ...
  require('../../src/assets/js/translations.js');
});
...
describe('TranslationManager', () => {
  ...
  beforeEach(() => {
    tm = new global.TranslationManager();
  ...
  describe('language switching', () => {
    it('should switch language successfully', async () => {
      ...
      tm.translationsCache.set('product-en', {});
    ...
    it('should fallback to default language if target language fails', async () => {
      ...
      // loadUITranslations fails for en; fallback zh-CN succeeds via mock
      jest.spyOn(tm, 'loadUITranslations')
        .mockRejectedValueOnce(new Error('Failed'))
        .mockResolvedValue({});
      // loadProductTranslations silently falls through (warn only)
      jest.spyOn(tm, 'loadProductTranslations').mockResolvedValue({});
  ...
  describe('cache management', () => {
    it('should allow manually clearing cache entries', () => {
    ...
    it('should re-fetch after cache is cleared', async () => {
      ...
      // Clear localStorage to ensure fetch is called
      localStorage.removeItem('yukoli-translations-ui-en');

      // Set up fetch spy before first call
      ...
      // Clear memory cache (this simulates a fresh load)
      tm.translationsCache.clear();

      // Clear localStorage too to ensure fetch is called again
      localStorage.removeItem('yukoli-translations-ui-en');

      // Re-fetch - should call fetch again since both caches are cleared
      await tm.loadUITranslations('en');

      // fetch should have been called again after cache was cleared
