...
### Fixed
- 修复产品图片无法显示的 bug：`imageRecognitionKey` 来自产品数据 i18n 字段时为原始型号格式（如 `ESL-GB60_1`），未经 `modelToImageKey()` 转换直接使用，导致 `IMAGE_ASSETS` 查找失败、图片 404。现统一经 `modelToImageKey()` 转为 `snake_case` 小写格式
...
- [已弃用] 飞书多维表格数据同步（`scripts/generate-products-data-table.js`）
- [已弃用] Gemini API 批量翻译引擎（`scripts/unified-translator.js`），支持增量翻译
...
- 一键发布脚本（`scripts/release.js`）：版本管理 → 数据同步 → 翻译 → 打包 → 孤立分支推送
