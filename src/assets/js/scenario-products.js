...
(function () {
  ...
  var SCENARIO_PRODUCTS = {
    "small-restaurant": [
      "DLB-TBS30", "DLB-TBQ30", "DLB-TQBQ30",
      "DLB-TGS30", "DLB-TGD30", "DLB-TBS40",
      "DLB-TZS40", "F32F1C", "G26D1A",
      "Y12D1C", "GT1D1B", "B4RTD",
    ],
    "chain-restaurant": [
      "DLB-4BQ30", "DLB-GD30", "DLB-GD36",
      "DLB-GQ30", "DLB-GQ36", "DLB-GQ30T",
      "DLB-GB50", "DLB-GC50", "G50AAB",
      "G60EAC", "DLB-XC80", "DLB-QXC80",
      "DLB-PZJ100", "DLB-PZJ120",
    ],
    "central-kitchen": [
      "DLB-TGQ40J", "DLB-TGQ36J", "DLB-GC50",
      "DLB-GC60", "DLB-GQ50", "DLB-GQ60",
      "G30DAG", "G36DAG", "B4RTD",
      "B6RBD", "DLB-XC100", "LZ80D1B",
    ],
    canteen: [
      "DLB-GB70", "DLB-GC70", "DLB-GB80",
      "DLB-GC80", "DLB-GQ70", "G70EAC",
      "G80EAC", "DLB-XC100", "DLB-QXC100",
      "DLB-PZJ200", "Z8FCB/Z12FCB", "Z6FCB",
      "DLB-ZNT", "DLB-BXC800",
    ],
    "cloud-kitchen": [
      "DLB-TGS30", "DLB-TGD30", "DLB-TGQ30",
      "G26D1A", "G30D1A", "G26DAA",
      "F32F1C", "Y12D1C", "Y12D2C",
      "GT2D1B",
    ],
    "food-factory": [
      "DLB-GB90", "DLB-GC90", "DLB-GQ90",
      "DLB-GB80", "G80EAC", "DLB-XC120",
      "DLB-QXC120", "DLB-PZJ400", "DLB-BXC800",
      "HKQPJ500-VIII", "DLB-A6200", "HKJGJ380-VI",
    ],
    "menu-lab": [
      "DLB-TGQ30J", "DLB-TGQ36J", "DLB-TGQ40J",
      "G30D1T", "G26DAG", "G30DAG",
      "J40CBB", "F32F1C", "DLB-TZS40",
      "Y12D1C", "GT1D1B", "B4RTD",
    ],
  };
  ...
  function loadProducts(scenarioKey, callback, retries) {
    ...
    if (!modelList) { callback([]); return; }

    var table = window.PRODUCT_DATA_TABLE || [];
    var lookup = {};
    modelList.forEach(function (m) { lookup[m.toLowerCase()] = true; });
  ...
  function buildCard(p, device) {
    ...
    var catMap = { "翻炒系列": "stirfry", "炖煮系列": "stewing", "蒸煮系列": "steaming", "煎炸系列": "frying", "切配系列": "cutting", "辅助系列": "other" };
    ...
    var descClass = device === "pc" ? "text-sm text-slate-500 dark:text-slate-400 line-clamp-2 mb-4" : "text-xs text-slate-500 line-clamp-2 mb-3";
    ...
    return (
      ...
      ' hover:border-primary hover:shadow-lg transition-all duration-300 overflow-hidden" data-link="' + href + '">' +
      '<div class="aspect-[4/3] bg-white overflow-hidden cursor-pointer">' +
      (img
        ? '<img alt="' + escHtml(p.model) + '" class="w-full h-full object-contain ' + imgClass + ' group-hover:scale-105 transition-transform duration-300" src="' + escHtml(img) + '" loading="lazy">'
        : '<div style="font-size:' + imgSize + ';color:#d1d5db;display:flex;align-items:center;justify-content:center;height:100%">&##x1f4e6;</div>') +
      "</div>" +
      '<div class="' + padClass + '">' +
      (p.category
        ? '<a href="' + catHref + '" class="inline-block ' + badgeClass + ' rounded-full bg-primary/10 text-primary hover:bg-primary/20 transition-colors font-bold">' + escHtml(translateCategory(p.category)) + "</a>"
        : "") +
      '<h3 class="' + titleClass + '">' + escHtml(p.model) + "</h3>" +
      (typeof getProductField === "function" && getProductField(p, "name")
        ? '<p class="' + descClass + '">' + escHtml(getProductField(p, "name")) + "</p>"
        : '<div class="mb-4"></div>') +
      '<div class="flex justify-between items-center pt-2 border-t border-slate-100' + (isDark ? ' dark:border-slate-700' : '') + '">' +
      (p.power ? '<span class="' + powerClass + ' font-bold text-slate-400">' + escHtml(p.power) + "</span>" : "<span></span>") +
      '<a href="' + href + '" data-no-swup class="font-bold text-primary flex items-center gap-1 group-hover:gap-2 transition-all duration-300 hover:opacity-80" data-i18n="home_hw_learn_more" style="font-size:' + (device === 'pc' ? '0.875rem' : '0.75rem') + ';line-height:1.25rem">' +
  ...
  function render(container, products, device) {
    ...
    visProducts.forEach(function (p) { html += buildCard(p, device); });
    html += "</div>";

    if (hasMore) {
      var hiddenId = "scenario-hidden-" + device;
      html += '<div id="' + hiddenId + '" style="display:none" class="' + gridClass + ' mt-4">';
      restProducts.forEach(function (p) { html += buildCard(p, device); });
      html += "</div>";

      if (device === "mobile") {
        html += '<button id="scenario-load-more" class="w-full mt-3 py-2.5 rounded-xl border border-slate-300 text-sm font-bold text-primary hover:bg-primary hover:text-white hover:border-primary transition-all flex items-center justify-center gap-2" onclick="(function(){var h=document.getElementById(\'' + hiddenId + '\');var b=document.getElementById(\'scenario-load-more\');if(h&&b){h.style.display=\'\';b.style.display=\'none\';window.translationManager&&window.translationManager.applyTo(h.parentElement);}})()">' +
          '<span class="material-symbols-outlined text-lg">expand_more</span> ' +
          escHtml(tl("home_show_more", "更多产品")) + "</button>";
      } else {
        html += '<div class="flex justify-center mt-8">' +
          '<button class="px-6 py-2.5 rounded-full border-2 border-primary text-primary font-bold hover:bg-primary hover:text-white transition-all cursor-pointer text-sm" onclick="(function(){var h=document.getElementById(\'' + hiddenId + '\');var b=this;if(h.style.display===\'none\'){h.style.display=\'\';b.textContent=typeof window.uiText===\'function\'?window.uiText(\'home_hw_collapse\',\'收起 ▲\'):\'收起 ▲\'}else{h.style.display=\'none\';b.textContent=typeof window.uiText===\'function\'?window.uiText(\'home_hw_show_more\',\'查看更多产品 ▼\'):\'查看更多产品 ▼\'}})()" data-i18n="home_hw_show_more">' +
          tl("home_hw_show_more", "查看更多产品 ▼") + "</button></div>";
      }
    }
  ...
  document.addEventListener("spa:load", function () { init(); });
