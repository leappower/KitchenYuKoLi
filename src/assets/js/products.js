...
(function (global) {
  ...
  function revealMobileControlsOnCenteredCard() {
    ...
    cards.forEach(function (card) {
      ...
      if (dist < nearestDist) { nearestDist = dist; nearestCardWidth = rect.width; }
    });
  ...
  function renderMobileProductSideControls(showControls, disableControls) {
    ...
    if (prevBtn) prevBtn.addEventListener('click', function () { scrollMobileProducts(-1); });
    if (nextBtn) nextBtn.addEventListener('click', function () { scrollMobileProducts(1); });
  ...
  global.addEventListener('resize', getDebounce()(function () {
  ...
  function renderProductFilters() {
    ...
    filterBar.innerHTML = seriesFilters.map(function (sf) {
      var key = sf.key, filterKey = sf.filterKey;
  ...
  function renderProducts() {
    ...
    var filtered = currentFilter ? allProducts.filter(function (p) { return p.category === currentFilter; }) : allProducts;
    ...
    meta.innerHTML = [
      ...
      '<button type="button" data-page="' + (currentPage - 1) + '" class="product-meta-nav-btn ios-nav-btn w-full justify-start ' + (prevDisabled ? 'is-disabled' : '') + ' sm:w-auto sm:justify-center" ' + (prevDisabled ? 'disabled' : '') + ' aria-label="' + tr('product_prev_page', 'Previous page') + '">',
      ...
      '</button>',
      '<button type="button" data-page="' + (currentPage + 1) + '" class="product-meta-nav-btn ios-nav-btn w-full justify-end ' + (nextDisabled ? 'is-disabled' : '') + ' sm:w-auto sm:justify-center" ' + (nextDisabled ? 'disabled' : '') + ' aria-label="' + tr('product_next_page', 'Next page') + '">',
    ...
    meta.querySelectorAll('.product-meta-nav-btn[data-page]').forEach(function (btn) {
      btn.addEventListener('click', function () { goToPage(Number(btn.dataset.page)); });
    });
    ...
    grid.innerHTML = pageProducts.map(function (p) {
      var displayName = getProductI18nField(p, 'name', p.name) || (tr(getCategoryI18nKey(p.category), p.category) + ' ' + (p.model || '')).trim();
      ...
      return [
        ...
        '<div class="flex-1 min-w-0"><h3 class="text-[13px] sm:text-sm font-bold text-slate-900 dark:text-slate-100 leading-tight line-clamp-2">' + displayName + '</h3></div>',
        ...
        '<div class="flex items-center rounded-md bg-slate-50 dark:bg-slate-800/70 p-1 min-w-0"><p class="text-[10px] text-slate-500 dark:text-slate-400 truncate flex-shrink-0">' + tr('product_label_capacity_throughput', 'Capacity') + ':</p><p class="text-xs font-bold text-slate-800 dark:text-slate-100 truncate flex-1 min-w-0 ml-1">' + (throughput || '-') + '</p></div>',
        '<div class="flex items-center rounded-md bg-slate-50 dark:bg-slate-800/70 p-1 min-w-0"><p class="text-[10px] text-slate-500 dark:text-slate-400 truncate flex-shrink-0">' + tr('product_label_voltage_frequency', 'Voltage') + ':</p><p class="text-xs font-bold text-slate-800 dark:text-slate-100 truncate flex-1 min-w-0 ml-1">' + (voltage || frequency ? ((voltage || '-') + ' / ' + (frequency || '-')) : '-') + '</p></div>',
        '<div class="flex items-center rounded-md bg-slate-50 dark:bg-slate-800/70 p-1 min-w-0"><p class="text-[10px] text-slate-500 dark:text-slate-400 truncate flex-shrink-0">' + tr('product_label_min_order_qty', 'MOQ') + ':</p><p class="text-xs font-bold text-slate-800 dark:text-slate-100 truncate flex-1 min-w-0 ml-1">' + tr(minimumOrderQuantity || '-', minimumOrderQuantity || '-') + '</p></div>',
        '<div class="flex items-center rounded-md bg-slate-50 dark:bg-slate-800/70 p-1 min-w-0"><p class="text-[10px] text-slate-500 dark:text-slate-400 truncate flex-shrink-0">' + tr('product_label_launch_date', 'LaunchDate') + ':</p><p class="text-xs font-bold text-slate-800 dark:text-slate-100 truncate flex-1 min-w-0 ml-1">' + tr(launchDate || '2025', launchDate || '2025') + '</p></div>',
        '</div>',
        '<div class="grid grid-cols-1 gap-1 mb-1.5 shrink-0">',
        '<div class="flex items-center rounded-md bg-slate-50 dark:bg-slate-800/70 p-1 min-w-0"><p class="text-[10px] text-slate-500 dark:text-slate-400 truncate flex-shrink-0">' + tr('product_label_material', 'Material') + ':</p><p class="text-xs font-bold text-slate-800 dark:text-slate-100 truncate flex-1 min-w-0 ml-1">' + (material || '-') + '</p></div>',
        '<div class="flex items-center rounded-md bg-slate-50 dark:bg-slate-800/70 p-1 min-w-0"><p class="text-[10px] text-slate-500 dark:text-slate-400 truncate flex-shrink-0">' + tr('product_label_scene', 'Scene') + ':</p><p class="text-xs font-bold text-slate-800 dark:text-slate-100 truncate flex-1 min-w-0 ml-1">' + (scene || '-') + '</p></div>',
        '</div>',
        '<div class="mt-auto grid grid-cols-2 gap-1 shrink-0">',
        '<button data-action="show-popup" class="inline-flex h-[36px] items-center justify-center gap-1 rounded-lg border border-primary/20 bg-primary/5 px-2 py-1.5 text-xs font-bold text-primary hover:bg-primary/10 transition-colors"><span class="material-symbols-outlined text-[10px]">tune</span><span class="truncate">' + tr('product_optional_specs', 'Optional') + '</span></button>',
        '<button data-action="show-popup" class="inline-flex h-[36px] items-center justify-center gap-1 rounded-lg bg-primary px-2 py-1.5 text-xs font-bold text-white hover:bg-primary/90 transition-colors"><span class="material-symbols-outlined text-[10px]">request_page</span><span class="truncate">' + tr('product_request', 'Request') + '</span></button>',
        '</div></div></article>'
      ].join('');
    }).join('');
    ...
    if (mobileCarousel) {
      ...
      _mobileCtrlTouchHandler = function () { resetMobileCtrlFadeTimer(); };
      ...
      if (_mobileCtrlCenterRevealRaf) { global.cancelAnimationFrame(_mobileCtrlCenterRevealRaf); _mobileCtrlCenterRevealRaf = 0; }
      if (_mobileCtrlTouchHandler) { grid.removeEventListener('touchstart', _mobileCtrlTouchHandler); _mobileCtrlTouchHandler = null; }
      if (_mobileCtrlTouchEndHandler) { grid.removeEventListener('touchend', _mobileCtrlTouchEndHandler); _mobileCtrlTouchEndHandler = null; }
    }

    renderPagination(totalPages, { totalCount: orderedProducts.length, from: from, to: to, currentPageCount: currentPageCount });
    ...
    if (filterBar) {
      filterBar.querySelectorAll('button[data-filter]').forEach(function (btn) {
        btn.addEventListener('click', function () { filterProducts(btn.dataset.filter); });
      });
    }
  }
  ...
  function renderPagination(totalPages, pageStats) {
    ...
    if (isMobileProductCarousel()) { pagination.innerHTML = ''; return; }
    if (totalPages <= 1) { pagination.innerHTML = ''; return; }

    var allCount = pageStats && isFinite(pageStats.totalCount)
      ? pageStats.totalCount
      : (currentFilter ? getProducts().filter(function (p) { return p.category === currentFilter; }).length : getProducts().length);
    ...
    var currentPageCount = pageStats && isFinite(pageStats.currentPageCount) ? pageStats.currentPageCount : Math.max(0, to - from + 1);
    ...
    html += '<button data-page="' + (currentPage - 1) + '" class="pagination-btn inline-flex items-center gap-1 px-3 py-2 rounded-lg text-sm ' + (currentPage === 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-slate-200 dark:hover:bg-slate-700') + '" ' + (currentPage === 1 ? 'disabled' : '') + '><span class="material-symbols-outlined text-lg">chevron_left</span><span>' + tr('product_prev_page', 'Previous') + '</span></button>';
    ...
    html += '<button data-page="' + (currentPage + 1) + '" class="pagination-btn inline-flex items-center gap-1 px-3 py-2 rounded-lg text-sm ' + (currentPage === totalPages ? 'opacity-50 cursor-not-allowed' : 'hover:bg-slate-200 dark:hover:bg-slate-700') + '" ' + (currentPage === totalPages ? 'disabled' : '') + '><span>' + tr('product_next_page', 'Next') + '</span><span class="material-symbols-outlined text-lg">chevron_right</span></button>';
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var clearTranslationTimeout = function () { clearTimeout(translationTimeout); };
    if (global.translationManager) {
