    ...
    grid.innerHTML = pageProducts
      .map(function (p) {
        ...
        return [
          ...
          '<a href="/products/" + encodeURIComponent(p.model || p.name || "") + \" class="inline-flex h-[36px] items-center justify-center gap-1 rounded-lg border border-primary/20 bg-primary/5 px-2 py-1.5 text-xs font-bold text-primary hover:bg-primary/10 transition-colors"><span class="material-symbols-outlined text-[10px]">tune</span><span class="truncate">' +
