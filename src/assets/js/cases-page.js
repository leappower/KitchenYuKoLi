...
(function () {
  ...
  function initFilters() {
    ...
    $$('button[data-i18n="case_studies_all_cases"]').forEach(function (btn) { btn.setAttribute('data-filter', 'all'); });
    $$('button[data-i18n="case_studies_chain_restaurants"]').forEach(function (btn) { btn.setAttribute('data-filter', 'chain'); });
    $$('button[data-i18n="case_studies_catering"]').forEach(function (btn) { btn.setAttribute('data-filter', 'catering'); });
    $$('button[data-i18n="case_studies_ghost_kitchens"]').forEach(function (btn) { btn.setAttribute('data-filter', 'ghost'); });
  ...
  function createModal() {
    ...
    modalOverlay.style.cssText = 'position:fixed;inset:0;z-index:9999;display:none;align-items:center;justify-content:center;padding:1rem;';
    modalOverlay.innerHTML = '<div style="position:absolute;inset:0;background:rgba(0,0,0,0.6);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);"></div>';
    modalOverlay.addEventListener('click', function (e) {
      if (e.target === modalOverlay || e.target === modalOverlay.firstElementChild) closeModal();
    });

    modalCard = document.createElement('div');
    modalCard.style.cssText = 'position:relative;max-width:768px;width:100%;max-height:90vh;overflow-y:auto;background:#fff;border-radius:1rem;box-shadow:0 25px 50px rgba(0,0,0,0.25);transform:scale(0.95);opacity:0;transition:transform 0.3s ease,opacity 0.3s ease;';
  ...
  function openModal(caseId) {
    ...
    var resultsHTML = c.results.map(function (r) {
      return '<div style="text-align:center;padding:1rem 0.5rem;background:#f8fafc;border-radius:0.75rem;" class="dark:bg-slate-800">' +
        '<div style="font-size:2rem;font-weight:900;color:var(--color-primary,#f97316);">' + r.value + '</div>' +
        '<div style="font-size:0.75rem;color:#64748b;font-weight:600;margin-top:0.25rem;" class="dark:text-slate-400">' + r.label + '</div>' +
        '</div>';
    }).join('');

    var equipHTML = c.equipment.map(function (e) {
      return '<div style="display:flex;justify-content:space-between;padding:0.5rem 0;border-bottom:1px solid #f1f5f9;" class="dark:border-slate-700">' +
        '<span><strong class="text-primary">' + e.model + '</strong> · ' + e.name + '</span>' +
        '<span style="font-weight:700;">' + e.qty + '</span>' +
        '</div>';
    }).join('');
    ...
    modalCard.innerHTML =
      ...
      '<div style="padding:2rem;">' +
        ...
        '<div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:1rem;">' +
          ...
          '<div>' +
            '<h2 style="font-size:1.5rem;font-weight:900;line-height:1.2;">' + c.title + '</h2>' +
            '<p style="color:#64748b;font-size:0.875rem;font-weight:600;" class="dark:text-slate-400">' + c.subtitle + '</p>' +
          '</div>' +
        '</div>' +
        '<p style="color:#475569;margin-bottom:1.5rem;" class="dark:text-slate-300">' + c.client + '</p>' +
        ...
        '<div style="margin-bottom:1.5rem;">' +
          ...
          '<p style="line-height:1.7;">' + c.challenge + '</p>' +
        '</div>' +
        ...
        '<div style="margin-bottom:1.5rem;">' +
          ...
          '<p style="line-height:1.7;">' + c.solution + '</p>' +
        '</div>' +
        ...
        '<div style="margin-bottom:1.5rem;">' +
          ...
          '<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:0.75rem;">' + resultsHTML + '</div>' +
        '</div>' +
        ...
        '<div style="background:#f8fafc;padding:1.25rem;border-radius:0.75rem;border-left:4px solid var(--color-primary,#f97316);" class="dark:bg-slate-800">' +
          '<p style="font-style:italic;line-height:1.7;margin-bottom:0.5rem;color:#334155;" class="dark:text-slate-300">"' + c.testimonial.text + '"</p>' +
          '<p style="font-size:0.8125rem;color:#64748b;" class="dark:text-slate-400">（' + c.testimonial.textCn + '）</p>' +
          '<p style="font-weight:700;margin-top:0.75rem;font-size:0.875rem;">— ' + c.testimonial.author + '</p>' +
          '<p style="font-size:0.8125rem;color:#64748b;" class="dark:text-slate-400">' + c.testimonial.role + '</p>' +
        '</div>' +
  ...
  function initCaseStudyButtons() {
    ...
    Object.keys(caseMap).forEach(function (titleKey) {
      ...
      $$('[data-i18n="' + titleKey + '"]').forEach(function (titleEl) {
        ...
        if (readBtn) {
          readBtn.addEventListener('click', function () { openModal(caseId); });
        }
      });
    });
    ...
    $$('button[data-i18n="featured_cta"]').forEach(function (btn) {
      btn.addEventListener('click', function () { openModal(1); });
    });
  }
  ...
  function initVideoButton() {
    $$('[data-i18n="case_studies_watch_video"]').forEach(function (el) {
      ...
      clickable.addEventListener('click', function () { openVideoModal(); });
    });
  }

  function openVideoModal() {
    if (videoOverlay) { videoOverlay.style.display = 'flex'; document.body.style.overflow = 'hidden'; return; }
    videoOverlay = document.createElement('div');
    videoOverlay.style.cssText = 'position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;padding:1rem;background:rgba(0,0,0,0.7);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);';
    ...
    videoOverlay.addEventListener('click', function (e) {
      if (e.target === videoOverlay) { videoOverlay.style.display = 'none'; document.body.style.overflow = ''; }
    });
    document.body.appendChild(videoOverlay);
    document.body.style.overflow = 'hidden';
    $('#video-modal-close').addEventListener('click', function () { videoOverlay.style.display = 'none'; document.body.style.overflow = ''; });
  }
