...
(function (global) {
  ...
  var smartPopup = {
    ...
    setupTracking: function () {
      ...
      document.addEventListener("click", function (e) {
        ...
        var productIntentTarget = e.target.closest(
  ...
  function submitSmartPopupForm(event) {
    ...
    var form = document.getElementById("smart-popup-form");
    if (!form) {
      showNotification(tr("notify_form_not_found", "Form not found"), "error");
      return;
    }
    var formData = {
    ...
    fetch(FORM_ENDPOINT, {
      ...
      .catch(function (error) {
        ...
        showNotification(
          tr("notify_submit_received", "Submitted successfully! We have received your information."),
          "success"
        );
        form.reset();
        setTimeout(function () {
  ...
  function submitViaMailto(formData, formType) {
    var screenWidth =
      global.DeviceUtils && global.DeviceUtils.getScreenSize ? global.DeviceUtils.getScreenSize() : global.screen.width;
    var subject = encodeURIComponent(
      (formType === "smart_popup"
        ? tr("mailto_subject_smart_popup", "Smart Popup")
        : tr("mailto_subject_contact_form", "Contact Form")) +
        " " +
        tr("mailto_subject_inquiry", "Inquiry") +
        " - " +
        formData.name
    );
    var body = encodeURIComponent(
      [
        ...
        tr("mailto_label_time_on_page", "Time on Page") +
          ": " +
          (userState.timeOnPage || 0) +
          tr("mailto_unit_seconds", "s"),
