...
(function (global) {
  ...
  function buildItem(sub, href) {
    ...
    var emojiHtml = sub.emoji
      ? '<span class="sup-dropdown-emoji">' + sub.emoji + '</span>'
      : '';
    return '<a href="' + esc(href) + '" class="sup-dropdown-item">' +
      '<span class="sup-dropdown-icon">' +
        '<span class="material-symbols-outlined">' + esc(sub.icon) + '</span>' +
      '</span>' +
      '<span class="sup-dropdown-label" data-i18n="' + esc(sub.key) + '">' + esc(sub.key) + '</span>' +
      emojiHtml +
      chevron +
    '</a>';
  }
  ...
  function renderDropdown(cfg) {
    ...
    var html = '<div class="sup-dropdown-wrap' + (isTouch() ? ' touch-device' : '') + '">' +
      '<a href="' + esc(cfg.href) + '"' +
         ' class="' + esc(cfg.activeClass || '') + ' sup-dropdown-trigger"' +
         ' data-sup-trigger-label="' + esc(cfg.labelKey) + '">' +
  ...
  function initDropdownClick() {
    document.addEventListener('click', function () {
      document.querySelectorAll('.sup-dropdown-wrap.is-open')
  ...
  function openPopup(href) {
    ...
    var items = SUBSERIES.map(function (s) {
      ...
      var emojiHtml = s.emoji
        ? '<span class="sup-popup-emoji">' + s.emoji + '</span>'
        : '';
      return '<a href="' + esc(href) + '" class="sup-popup-item">' +
        '<span class="sup-dropdown-icon">' +
          '<span class="material-symbols-outlined">' + esc(s.icon) + '</span>' +
        '</span>' +
        '<span class="sup-popup-label" data-i18n="' + esc(s.key) + '">' + esc(s.key) + '</span>' +
        emojiHtml +
        chevron +
      '</a>';
    }).join('\n');
  ...
  function closePopup() {
    document.querySelectorAll('.sup-popup-overlay,.sup-popup-panel')
      .forEach(function (el) {
