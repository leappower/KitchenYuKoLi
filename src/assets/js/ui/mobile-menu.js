...
(function (global) {
  ...
  var MENU_ITEMS = [
    {
      key: 'nav_products',  href: '/products/',  id: 'products',
      icon: 'kitchen',
      children: [
    ...
    },
    {
      ...
      children: [
        ...
        { key: 'nav_contact_whatsapp',  icon: 'chat',        href: 'https://wa.me/XXXXXXXXXXX', isWhatsApp: true },
      ],
    },
  ];
  ...
  function buildMenuPanel() {
    ...
    var headerHtml =
      '<div class="mobile-menu-header">' +
        '<a class="mobile-menu-logo" href="' + (window.BASE_PATH || '') + '/home/">' +
          '<img src="' + (window.BASE_PATH || '') + '/assets/images/logo_footer.webp" alt="Yukoli" width="32" height="32" />' +
        '</a>' +
    ...
    var menuHtml = MENU_ITEMS.map(function (item) {
      ...
      if (item.children && item.children.length > 0) {
        var itemsHtml = item.children.map(function (child) {
          ...
          return '<a href="' + esc(child.href) + '" class="mobile-menu-l2-item' + waCls + '"' + targetAttr + '>' +
            '<span class="mobile-menu-l2-icon">' +
              '<span class="material-symbols-outlined">' + esc(child.icon) + '</span>' +
            '</span>' +
            '<span class="mobile-menu-l2-label" data-i18n="' + esc(child.key) + '">' + esc(child.key) + '</span>' +
            badgeHtml +
          '</a>';
        }).join('\n');

        childrenHtml =
          '<div class="mobile-menu-l2" data-menu-l2="' + esc(item.id) + '">' +
            itemsHtml +
          '</div>';
      }

      return '<div class="mobile-menu-l1-wrap">' +
        '<button class="mobile-menu-l1" data-menu-toggle="' + esc(item.id) + '" type="button">' +
          '<span class="mobile-menu-l1-icon">' +
            '<span class="material-symbols-outlined">' + esc(item.icon) + '</span>' +
          '</span>' +
          '<span class="mobile-menu-l1-label" data-i18n="' + esc(item.key) + '">' + esc(item.key) + '</span>' +
  ...
  function renderMobileSearchResults(results) {
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      html += '<a class="mobile-search-result-item" href="/products/">' +
        '<div class="mobile-search-result-img">' +
          (imgSrc ? '<img src="' + imgSrc + '" alt="" loading="lazy" onerror="this.style.display=\'none\'">' : '<span class="material-symbols-outlined">inventory_2</span>') +
        '</div>' +
        '<div class="mobile-search-result-info">' +
          '<div class="mobile-search-result-name">' + name + '</div>' +
