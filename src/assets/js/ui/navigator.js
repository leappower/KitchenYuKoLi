...
(function (global) {
  ...
  function buildNavLinks(activeId, variant) {
    return NAV_ITEMS.map(function (item) {
      ...
      return '<a class="' + cls + '" href="' + esc(href) + '" data-i18n="' + esc(item.key) + '">' + esc(item.key) + '</a>';
    }).join('\n');
  }
  ...
  function buildSearchBox(i18nKey, breakpoint) {
    ...
    return (
      '<div class="' + bp + ' ios-search-wrapper items-center">' +
  ...
  function buildCtaButton(textKey, href) {
    return (
      '<a href="' + esc(href) + '"' +
         ...
         ' data-i18n="' + esc(textKey) + '">' + esc(textKey) + '</a>'
    );
  }
  ...
  function mount() {
    ...
    }, true); // capture phase — runs before stopPropagation in bubble phase
  ...
  function updateActive(activeId) {
    ...
    var navLinks = document.querySelectorAll('header nav a[data-i18n], header nav a.prod-dropdown-trigger, header nav a.sol-dropdown-trigger, header nav a.app-dropdown-trigger, header nav a.sup-dropdown-trigger, header nav a.abt-dropdown-trigger, header nav a.cnt-dropdown-trigger');
    ...
    for (var i = 0; i < navLinks.length; i++) {
      var link = navLinks[i];
      var isActive = false;
      var linkKey = link.getAttribute('data-i18n') || link.getAttribute('data-prod-trigger-label') || link.getAttribute('data-sol-trigger-label') || link.getAttribute('data-app-trigger-label') || link.getAttribute('data-sup-trigger-label') || link.getAttribute('data-abt-trigger-label') || link.getAttribute('data-cnt-trigger-label') || '';
      ...
      var isDropdownTrigger = link.classList.contains('prod-dropdown-trigger') || link.classList.contains('sol-dropdown-trigger') || link.classList.contains('app-dropdown-trigger') || link.classList.contains('sup-dropdown-trigger') || link.classList.contains('abt-dropdown-trigger') || link.classList.contains('cnt-dropdown-trigger');
      if (isActive) {
        link.className = 'text-sm font-semibold text-primary' + (isDropdownTrigger ? ' prod-dropdown-trigger' : '');
      } else {
        link.className = 'text-sm font-semibold hover:text-primary transition-colors' + (isDropdownTrigger ? ' prod-dropdown-trigger' : '');
      }
    }
  }
