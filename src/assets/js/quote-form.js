    ...
    form.addEventListener("submit", function (e) {
      ...
      fetch(GAS_URL, {
        ...
        .then(function (res) {
          ...
          if (!res.ok) console.warn("[QuoteForm] GAS returned " + res.status);
        })
        .catch(function (err) {
          clearTimeout(timeout);
          if (err.name === "AbortError") console.error("[QuoteForm] Request timeout");
          else console.error("[QuoteForm] Submit error:", err.message);
        })
        .finally(function () {
          // 跳转到感谢页（无论成败）
          setTimeout(function () {
