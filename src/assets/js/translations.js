...
(function (global) {
  ...
  var langRegistry = global.LANG_REGISTRY
  ...
  TranslationManager.prototype.fetchTranslations = function (lang) {
    var self = this;
    return this.loadUITranslations(lang)
      .then(function (uiTranslations) {
  ...
  TranslationManager.prototype.loadUITranslations = function (lang) {
    ...
    if (typeof fetch !== "function")
      return lang !== "zh-CN" ? self.loadUITranslations("zh-CN") : Promise.reject(new Error("fetch not available"));

    var basePath = (typeof window !== "undefined" && window.BASE_PATH) || "";
    var fetchPromise = fetch(basePath + "/assets/lang/" + lang + "-ui.json", {
      cache: _isDevOrTest ? "no-store" : "default",
    })
  ...
  TranslationManager.prototype.loadProductTranslations = function (lang) {
    ...
    var basePath = (typeof window !== "undefined" && window.BASE_PATH) || "";
    var fetchPromise = fetch(basePath + "/assets/lang/" + lang + "-product.json", {
      cache: _isDevOrTest ? "no-store" : "default",
    })
      .then(function (response) {
  ...
  TranslationManager.prototype.updateCurrentLanguageLabel = function (langCode) {
    ...
    if (this.languages && Array.isArray(this.languages)) {
      var langObj = this.languages.find(function (l) {
  ...
  TranslationManager.prototype.bindDropdownEvents = function () {
    ...
    // 搜索输入过滤
    var searchInput = dropdown.querySelector(
      '#lang-search-input, input[data-i18n-placeholder="lang_search_placeholder"]'
    );
    if (searchInput) {
  ...
  TranslationManager.prototype.setupEventListeners = function () {
    ...
    if (allToggleBtns.length > 1) {
      console.warn(
        "[i18n] setupEventListeners: found " +
          allToggleBtns.length +
          ' elements with id="lang-toggle-btn".' +
  ...
  global.toggleLanguageDropdown = function (event) {
    return translationManager.toggleLanguageDropdown(event);
  };
  global.filterLanguages = function (query) {
    return translationManager.filterLanguages(query);
  };
  global.setupLanguageSystem = function () {
  ...
  // Debug helpers
  global.debugTranslations = function () {
    return translationManager.debug();
  };
  global.reloadTranslations = function () {
    return translationManager.reloadTranslations();
  };
  global.recoverTranslationsFromBfcache = function () {
    return translationManager.recoverFromBfcache();
  };
