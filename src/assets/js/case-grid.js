...
(function () {
  ...
  var ROI_CASES = [
    {
      slug: "manila-small-resto-labor-62",
      country: "🇵🇭 Philippines",
      industry: "小型餐饮",
      volume: "200-500",
    ...
    {
      slug: "kl-canteen-2000-meals",
      country: "🇲🇾 Malaysia",
      industry: "智慧食堂",
      volume: "1000+",
      benefit: "Fast Payback",
      dailyOutput: 2000,
      laborBefore: 15,
      laborAfter: 6,
    ...
    {
      slug: "surabaya-central-automation",
      country: "🇮🇩 Indonesia",
      industry: "中央厨房",
      volume: "500-1000",
      benefit: "Labor Cost Reduction",
  ...
  function renderCardPc(c) {
    ...
    return (
      '<div class="case-card bg-white dark:bg-slate-800 rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all group border border-slate-200 dark:border-slate-700 hover:border-' +
      bc +
      '-500/50">' +
      "<!-- TODO: 替换为 " +
      c.slug +
      ...
      "</div>" +
      '<div class="text-xl font-black text-' +
      bc +
      '-600">人工 -' +
      pct +
      "%</div>" +
      ...
      " 个月回本</span>" +
      "</div>" +
      '<p class="text-base leading-relaxed" data-i18n="cases_quote_' +
      c.slug +
      '">' +
      c.quote +
      "</p>" +
      '<h3 class="font-bold text-lg leading-snug" data-i18n="cases_title_' +
      c.slug +
      '">' +
      c.title +
      "</h3>" +
      '<a href="/cases/' +
      c.slug +
      '/" class="inline-flex items-center gap-1 text-primary font-bold text-sm group-hover:gap-2 transition-all mt-auto">' +
  ...
  function renderCardMobile(c) {
    ...
    return (
      ...
      '<span class="inline-flex items-center gap-1 text-sm font-bold text-' +
      bc +
      '-600">' +
      ...
      '<a href="/cases/' +
      c.slug +
      '/" class="inline-flex items-center gap-1 text-primary font-bold text-sm">' +
  ...
  function renderGrid(variant) {
    ...
    if (countEl) countEl.textContent = cases.length + " Case Studies";
  }
  ...
  function buildFiltersMobile() {
    ...
    html +=
      '<span id="case-count" class="flex-shrink-0 text-xs font-bold text-primary whitespace-nowrap">8 Case Studies</span>';
    html += "</div>";
    bar.innerHTML = html;
