...
(function() {
  ...
  CMS.openExcelImport = function() {
    var html = '<div style="display:flex;flex-direction:column;gap:1rem">' +
      ...
      '<div class="flex items-center justify-between mb-2"><span class="text-sm font-medium text-gray-700" id="excel-count"></span>' +
      '<span class="text-xs text-gray-400" id="excel-img-info"></span></div>' +
    ...
    showModal('excel-import-modal', '📥 导入 Excel 产品数据', html, function() {
      ...
      .then(function(d) {
        ...
        if (CMS.refreshProducts) CMS.refreshProducts();
      })
    ...
    setTimeout(function() {
      ...
      previewBtn.addEventListener('click', function() {
        ...
        .then(function(d) {
          ...
          var imgInfo = document.getElementById('excel-img-info');
          var table = document.getElementById('excel-table');
          if (!preview) return;
          preview.style.display = '';
          if (count) count.textContent = d.products_found + ' 个产品';
          if (imgInfo) {
            var ic = d.image_count || 0;
            imgInfo.textContent = ic > 0 ? '📊 包含 ' + ic + ' 张嵌入图片' : '';
          }
          var prods = d.products || [];
          var imgCount = d.image_count || 0;
          var html = '<table style="width:100%;font-size:0.75rem;border-collapse:collapse">' +
            ...
            '<th style="padding:6px 8px;text-align:center;border-bottom:1px solid #e5e7eb">图片</th>' +
            '</tr></thead><tbody>';
          prods.slice(0, 50).forEach(function(p) {
            var imgCell = '<span style="color:#d1d5db">—</span>';
            html += '<tr>' +
              ...
              '<td style="padding:4px 8px;border-bottom:1px solid #f3f4f6;text-align:center">' + imgCell + '</td>' +
              '</tr>';
          });
          if (prods.length > 50) html += '<tr><td colspan="5" style="padding:8px;text-align:center;color:#9ca3af">...还有 ' + (prods.length - 50) + ' 个产品</td></tr>';
          html += '</tbody></table>';
          table.innerHTML = html;
          toast('解析完成: ' + d.total_rows + ' 行，识别 ' + d.products_found + ' 个产品' + (imgCount > 0 ? '，' + imgCount + ' 张图片' : ''));
        })
        .catch(function(e) {
          previewBtn.disabled = false;
          previewBtn.textContent = '👁️ 预览数据（不导入）';
