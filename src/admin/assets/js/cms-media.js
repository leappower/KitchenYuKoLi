...
(function() {
  ...
  CMS.renderMedia = function(area) {
    var typeFilterHtml = '<div class="flex items-center gap-3">' +
      '<span class="text-xs text-gray-400">类型:</span>' +
      '<button class="btn-primary media-type-btn active" data-filter="all" style="font-size:0.8rem;padding:0.35rem 0.85rem;border-radius:9999px">全部</button>' +
      '<button class="btn-ghost media-type-btn" data-filter="image" style="font-size:0.8rem;padding:0.35rem 0.85rem;border-radius:9999px">🖼️ 图片</button>' +
      '<button class="btn-ghost media-type-btn" data-filter="video" style="font-size:0.8rem;padding:0.35rem 0.85rem;border-radius:9999px">🎬 视频</button>' +
      '<button class="btn-ghost media-type-btn" data-filter="pdf" style="font-size:0.8rem;padding:0.35rem 0.85rem;border-radius:9999px">📄 PDF</button>' +
      '</div>';

    var catFilterHtml = '<div class="flex items-center gap-3 flex-wrap" id="category-filters">' +
      '<span class="text-xs text-gray-400">分类:</span>' +
      '<button class="btn-primary media-cat-btn active" data-cat="all" style="font-size:0.8rem;padding:0.35rem 0.85rem;border-radius:9999px">全部</button>' +
      '</div>';
    ...
    area.innerHTML = '<div class="fade-in">' +
      ...
      '<label class="btn-primary" style="cursor:pointer;display:inline-flex;align-items:center;gap:6px;font-size:0.85rem;padding:0.4rem 0.9rem;border-radius:8px">+ 上传文件' +
      '<input type="file" multiple accept="image/*,video/mp4,.pdf" style="display:none" id="media-upload"></label>' +
      '</div></div>' +
      '<div class="flex flex-col gap-4 mb-6">' + typeFilterHtml + catFilterHtml + '</div>' +
    ...
    api('/media/categories').then(function(d) {
      ...
      d.categories.forEach(function(cat) {
        ...
        btn.style.cssText = 'font-size:0.8rem;padding:0.35rem 0.85rem;border-radius:9999px;margin:0 0.2rem';
