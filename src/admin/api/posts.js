...
function postsRoutes(db) {
  ...
  router.get('/posts', (req, res) => {
    try {
      ...
      var offset = (pg - 1) * lim;
      var rows = db.prepare('SELECT * FROM posts ' + whereClause + ' ORDER BY sort_order ASC, published_at DESC LIMIT ? OFFSET ?').params(...params, lim, offset).all();
      var countRow = db.prepare('SELECT COUNT(*) as total FROM posts ' + whereClause).params(...params).get();
  ...
  router.get('/posts/:id', (req, res) => {
    try {
      var row = db.prepare('SELECT * FROM posts WHERE id = ?').params(req.params.id).get();
  ...
  router.post('/posts', requireAdmin, (req, res) => {
    try {
      ...
      var result = db.prepare('INSERT INTO posts (title, slug, excerpt, content_markdown, cover_image, category, sort_order, published_at) VALUES (?, ?, ?, ?, ?, ?, ?, datetime("now"))').params(title, slug, excerpt || '', content_markdown || '', cover_image || '', category || 'news', parseInt(sort_order) || 0).run();
      var post = db.prepare('SELECT * FROM posts WHERE id = ?').params(result.lastInsertRowid).get();
  ...
  router.delete('/posts/:id', requireAdmin, (req, res) => {
    try {
      var existing = db.prepare('SELECT * FROM posts WHERE id = ?').params(req.params.id).get();
      if (!existing) return res.status(404).json({ error: 'Post not found' });
      db.prepare('DELETE FROM posts WHERE id = ?').params(req.params.id).run();
  ...
  router.post('/posts/:id/publish', requireAdmin, (req, res) => {
    try {
      db.prepare('UPDATE posts SET published_at = datetime("now"), is_active = 1, updated_at = datetime("now") WHERE id = ?').params(req.params.id).run();
      var post = db.prepare('SELECT * FROM posts WHERE id = ?').params(req.params.id).get();
